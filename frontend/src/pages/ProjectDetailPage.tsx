...
import { User, Project, TaskItem, TaskComment, TaskAttachment, RoleRequirement } from '../types';
...
export default function ProjectDetailPage({ currentUser }: ProjectDetailPageProps) {
  ...
  const handleUploadAttachment = async (e: React.ChangeEvent<HTMLInputElement>, targetTaskId?: string) => {
    ...
    if (!currentTaskId) {
      // It's a new task creation in project, add to pending files
      const ext = file.name.split('.').pop()?.toLowerCase() || '';
      const isImage = ['jpg', 'jpeg', 'png'].includes(ext);
      const isDoc = ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'txt', 'csv'].includes(ext);
    ...
    try {
      ...
      setProject((prev) => {
        ...
        const updatedTasks = (prev.tasks || []).map((t) =>
          t.id === currentTaskId ? { ...t, attachments: [newAttachment, ...(t.attachments || [])] } : t
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
        <div className="flex border-b border-slate-200 bg-slate-50/70 px-4 sm:px-6 pt-3 space-x-4 sm:space-x-6 overflow-x-auto whitespace-nowrap">
          <button
            onClick={() => setActiveTab('docs')}
            className={`pb-3 text-xs font-bold border-b-2 transition-all flex items-center space-x-2 ${activeTab === 'docs'
          ...
          <button
            ...
            className={`pb-3 text-xs font-bold border-b-2 transition-all flex items-center space-x-2 ${activeTab === 'team'
                ? 'border-indigo-600 text-indigo-600'
                : 'border-transparent text-slate-500 hover:text-slate-800'
              }`}
          >
          ...
          {project.canAccessContent && (
            <button
              ...
              className={`pb-3 text-xs font-bold border-b-2 transition-all flex items-center space-x-2 ${activeTab === 'tasks'
                  ? 'border-indigo-600 text-indigo-600'
                  : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
            >
          ...
          {isAdmin && (
            <button
              ...
              className={`pb-3 text-xs font-bold border-b-2 transition-all flex items-center space-x-2 ${activeTab === 'logs'
        ...
        <div className="p-6 text-xs text-slate-700">
          ...
          {activeTab === 'tasks' && project.canAccessContent && (() => {
            ...
            const archivedProjectTasksCount = allProjectTasks.filter((t) => Boolean(t.isArchived)).length;
            const tasksList = allProjectTasks.filter((t) => (showArchivedTasks ? Boolean(t.isArchived) : !t.isArchived));
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center space-x-3 flex-wrap gap-y-2">
                    ...
                    <button
                      ...
                      className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${showArchivedTasks
                          ? 'bg-amber-600 text-white shadow-xs'
                          : 'bg-slate-100 hover:bg-slate-200 text-slate-700'
                        }`}
                      title={showArchivedTasks ? 'Aktif görevleri göster' : 'Arşivlenmiş görevleri listele'}
                    >
                      {showArchivedTasks ? (
                        <>
                    ...
                    {statusColumns.map((col) => {
                      ...
                      return (
                        <div
                          ...
                          className={`border rounded-2xl p-3.5 flex flex-col min-h-[450px] transition-all duration-150 ${isOver
                            ...
                            {colTasks.map((t) => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`bg-white border rounded-xl p-3.5 shadow-xs hover:shadow-md transition-all space-y-3 ${!isSpectator ? 'cursor-grab active:cursor-grabbing' : ''} select-none ${isDragging
                            ...
                            <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                              <td className="py-3 px-4">
                                <div className="font-bold text-slate-900 text-xs flex items-center space-x-1.5 flex-wrap gap-y-1">
                                  {t.isArchived && (
                                    <span className="bg-amber-50 text-amber-800 border border-amber-200 text-[9.5px] font-bold px-1.5 py-0.5 rounded-md flex items-center space-x-1 shrink-0">
                                      <Archive className="w-2.5 h-2.5 text-amber-600" />
                                      <span>Arşivde</span>
                                    </span>
                                  )}
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
            <div className="w-screen max-w-full sm:max-w-lg md:max-w-xl bg-white shadow-2xl border-l border-slate-200 flex flex-col h-full animate-in slide-in-from-right duration-200 text-xs text-slate-700">
              ...
              <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end space-x-3 shrink-0">
                ...
                >
                  {editingTask ? 'Görevi Güncelle' : 'Görevi Projeye Kaydet'}
                </button>
              </div>
            </div>
          </div>
                                  ...
                                  >
                                    ...
                                    <div className="p-2 flex items-center justify-between bg-white" onClick={(e) => e.stopPropagation()}>
                                      ...
                                      <div className="flex items-center space-x-1 shrink-0">
                                        ...
                                        {(isAdmin || isProjectModerator || att.userId === currentUser.id) && (
                                          <button
